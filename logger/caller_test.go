...
func TestAccessLoggerDirectInfoKeepsDefaultCallerBehavior(t *testing.T) {
	baseCore, observed := observer.New(zapcore.InfoLevel)
	baseLogger := zap.New(baseCore, zap.AddCaller())
	log := NewAccessLogger(baseLogger)

	writeAccessDirectLog(log)
	...
	if !strings.HasSuffix(entries[0].Caller.File, "logger/caller_test.go") {
		t.Fatalf("expected caller to point to direct call site, got %s", entries[0].Caller.File)
	}
}
...
func TestServerLoggerDirectInfoKeepsDefaultCallerBehavior(t *testing.T) {
	baseCore, observed := observer.New(zapcore.InfoLevel)
	baseLogger := zap.New(baseCore, zap.AddCaller())
	log := NewServerLogger(baseLogger)

	writeServerDirectLog(log)
...
func writeAccessDirectLog(log *AccessLogger) {
	log.Info("access-direct")
}
...
func writeServerDirectLog(log *ServerLogger) {
	log.Info("server-direct")
}
